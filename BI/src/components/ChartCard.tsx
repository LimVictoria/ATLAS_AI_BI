...
function CodeFace({ metricId, sql, baseSql, color, light }: { metricId: string; sql?: string; baseSql?: string; color: string; light: string }) {
  ...
  const info = FALLBACK_SQL(metricId, sql)
    ...
    try {
      ...
      if (sourceSql) {
        ...
        updateChart(card.id, {
          ...
          sql: result.sql || sourceSql,  // sql = filtered version for flip panel
          filters: newFilters,
          loading: false,
        })
      } else {
